...
export class MovieService {
  ...
  updateByTitle(title: string, updatedData: Partial<CreateMovieDto>) {
    return this.movieRepository.update({ title, deleted: false }, updatedData);
  }
  ...
  async deleteById(id: number) {
    ...
    const showtimes = await this.showtimeRepository.find({ where: { movieId: id, deleted: false } });
    ...
    for (const showtime of showtimes) {
      ...
      const bookings = await this.bookingRepository.find({ where: { showtimeId: showtime.id, deleted: false } });
    ...
    return { message: `Movie ${id} and related showtimes and bookings logically deleted.` };
  }
}
